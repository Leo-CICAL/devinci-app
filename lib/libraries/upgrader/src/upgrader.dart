...
class Upgrader {
  ...
  String buttonTitleIgnore = 'Ignorer'.toUpperCase();

  /// The later button title, which defaults to ```Later```
  String buttonTitleLater = 'Plus tard'.toUpperCase();

  /// The update button title, which defaults to ```Update Now```
  String buttonTitleUpdate = 'Mettre à jour'.toUpperCase();
  ...
  String prompt = 'Voulez vous mettre à jour maintenant ?';
      ...
      if (bestItem != null &&
        ...
        if (debugLogging) {
          print(
              ...
              print('description : ${bestItem.itemDescription}');
        }
        _appStoreVersion ??= bestItem.versionString;
        _appStoreListingURL ??= bestItem.fileURL;
        _appStoreDescription ??= bestItem.itemDescription;
...
//      }
  ...
  String description() {
    return _appStoreDescription;
  }

  String message() {
    return 'Une nouvelle version de  ${appName()} est disponible! La version ${currentAppStoreVersion()} est disponible.\nVous avez actuellement la version ${currentInstalledVersion()}.\n\nChangements:\n${description().replaceAll("â¢", "•")}';
  }
