...
module.exports = {
    ...
    run: async (env, client, message, args) => {
        ...
        var config = fs.readFileSync(env.STORAGE_PATH+"sondage_config.json");
        ...
        if (args[0] == "add" && message.author.id == "251801773486899210"){
            ...
            let sondageembed = {
                description: "",
                color: env.SONDAGE_COLOR,
                footer: "Sondage fait par " + message.author.username + " #" + config.nbsondage
            };
            message.guild.channels.create("sondage-" + config.nbsondage, {
                type: "text",
                parent: message.guild.channels.cache.get(env.SONDAGE_CATEGORY),
                ...
                cancelcollect.on("collect", () =>{
                    cancelcollect.stop();
                    collector.stop();
                    inprogress = CancelSondage(env, config, message, client)
                    channel.delete();
                })
                ...
                collector.on('collect', m => {
                    ...
                    message.guild.channels.cache.get(channel.id).send(createsondage).then(msg => {
                        ...
                        collectormsg.on('collect', m => {
                            ...
                            Promise.all(promises).then(() => {
                                message.guild.channels.cache.get(env.SONDAGE_CHANNEL).send(new MessageEmbed(sondageembed)).then(endmsg => {
                                    for (let i = 0; option.length > i; i++) {
                                        switch (i) {
                                            case 0:
                                                endmsg.react(`0️⃣`)
